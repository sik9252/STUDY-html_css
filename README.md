...
<과정>
ⓐ button 태그를 이용해 버튼을 생성하고 스타일링을 위해 shake-btn이라는 클래스를 부여했다.

ⓑ css 파일을 생성해 ⓐ에서 생성한 버튼의 기본 스타일(크기, 색상 등)을 만들어 주었다.

ⓒ 복잡하진 않지만 한번의 hover에 여러 애니메이션을 줘야 하므로 @keyframes를 이용해 각 %마다 세부 애니메이션을 지정해주었다.

ⓓ shake-btn:hover 안에 animation-name, animation-duration을 이용해 버튼에 마우스를 올렸을때ⓒ에서 만든 애니메이션을 수행할수 있도록 넣어주었다.
...
조건: 좌측으로 살짝 회전했다가 우측으로 회전하면서 커짐

<과정>

ⓐ 키보드의 +기호를 이용해 버튼을 생성한다.

ⓑ css파일에 @keyframes를 이용해 rotate라는 애니메이션을 만든 후 각각의 %에 해당하는 애니메이션을 지정해준다.

ⓒ 애니메이션 진행이 100%일때 + 기호가 우측으로 회전하면서 크기가 커져야하기 때문에 rotate와 scale 속성을 같이 적어주었다.

ⓓ 생성한 애니메이션을 +기호 버튼이 hover 되었을때 적용될수 있도록 +기호 버튼 클래스:hover에 animation-name과 animation-duration을 적어준다.

ⓔ 이때 animation-fill-mode: forwards라는 속성도 적어주었는데 이것을 적용하니까 애니메이션이 끝나도 100%에서 적용된 애니메이션 상태에 멈춰있었다. 이것을 제거하면 자동으로 0%대로 돌아간다.

**<완성본>**

![rotateplus](https://user-images.githubusercontent.com/64947440/149284734-2e1af74a-c2d5-4608-9e79-ad04138717cb.gif)
